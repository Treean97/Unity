...
Shader "Custom/shCustomAlphaBlend_2"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Blend One One
        // <- SRCCOLOR*1 + DSTCOLOR1 = SRCCOLOR*1 + DSTCOLOR*1
        // 이러한 경우 src의 컬러 색상이 검정색(0,0,0)이면 뒤에 배경이 온전히 보인다
        // 그러므로 검정색상이 알파의 역할을 하게 된다

        CGPROGRAM
        // Physically based Standard lighting model, and enable shadows on all light types
        #pragma surface surf Lambert keepalpha noambient
        // 텍스처 이미지 데이텅 ㅔ포함된 알파값을 그대로 유지하여 반영한다
