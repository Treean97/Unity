    ...
    컴퓨터의 실수 표현
        ...
        <--
            장점: 수를 해석하는데 들어가는 연산이 비교적 적다.
                왜냐하면, 각각의 정수부, 소수점 이하부, 부호비트를 담당하는 비트들을 그냥 그대로 해석하기 때문이다.
            단점: 정해진 비트들을 그냥 그대로 해석하므로 
                정밀도가 낮다.
...
Shader "Ryu/shRyuOpColor"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
            ...
            //o.Emission = fixed2(1, 0) + fixed4(0, 1, 0, 1); //자릿수가 다르면 연산이 불가능하다.
            //o.Emission = fixed2(1, 0) + fixed3(0, 1, 0);//자릿수가 다르면 연산이 불가능하다.
            //o.Emission = fixed2(1, 0)*fixed3(0, 1, 0);//자릿수가 다르면 연산이 불가능하다.

            //--한 자릿수는 모든 자릿수에 연산이 가능하다.!!! (1*0.5, 0*0.5, 0*0.5, 1*0.5) = (0.5, 0, 0, 0.5)
            //o.Emission = fixed(0.5f) * fixed4(1, 0, 0, 1);

            //--한 자릿수는 모든 자릿수에 연산이 가능하다.!!! (1+0.5, 0+0.5, 0+0.5, 1+0.5) = (1.5, 0.5, 0.5, 1.5)
