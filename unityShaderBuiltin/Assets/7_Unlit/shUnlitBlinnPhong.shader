    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            struct v2f
            {
                ...
                float4 mSpecular : TEXCOORD1;   // 정반사광 전달을 위함, 짝수로 맞추기 위해 float4
            };
            ...
            v2f vert (appdata v)
            {
                ...
                o.mSpecular = pow(tDotResult,30);

                return o;
            }

            // 랜더링 파이프라인을 모두 거친 픽셀 데이터들은
            // 더블버퍼(back, front buffer 두개로 이루어진다) 시스템의
            // 백버퍼에 그려지게 된다
            // 여기에서 이 백버퍼를 흔히 Target이라 한다
