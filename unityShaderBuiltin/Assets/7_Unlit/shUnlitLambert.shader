/*
    ...
    Pass
    '렌더링에 필요한 데이터들을 조직화하고
    렌더링에 필요한 상태들을 설정하고
    렌더링 에서 그리기 명령 한번 호출'을 'pass'라고 한다.

    <---즉, 렌더링에 사용할 색상, 텍스처, 조명 등등을 설정하고 
...
Shader "Ryu/shUnlitLambert"
{
    ...
    SubShader
    {
        ...
        Pass//<---
        {
            ...
            /*
                fragment: 
                    스크린에 표시되는 pixel이 되기 전까지의 단계의 데이터를 원래는 fragment라고 불렀다.
            ...
            */
            ...
            struct appdata      //응용프로그램에서 전달받은 데이터
            {
                ...
                float2 uv : TEXCOORD0;//텍스처좌표정보 슬롯 첫번째

                //조명 연산을 하기 위해 법선 데이터가 필요하므로 appdata에 명시해준다.
            ...
            struct v2f  //<-- from vertex to fragment
            {
                ...
                //광량을 전달하기 위해 변수를 만듦, 정해진 시맨틱을 적용하기 위해 COLOR0을 선택, 그리고 컬러이므로 float4로 타입 선택
