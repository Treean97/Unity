Shader "Ryu/shOutlineRim"
{
    ...
    SubShader
    {
        ...
        float4 LightingRyuOutlineRim(SurfaceOutputRyuOutlineRim s, float3 lightDir, float3 viewDir, float atten)
        {
            ...
            float tDotResult = abs(tDot);//[0,1] <--saturate를 썻을 때와 다른 결과이다.
            //예를 들면,
            ...
            //림라이트 효과에 해당하는 광량 수치를 구한다.
            //float tRim = 1 - tDotResult;

            //외곽선의 경계를 명확하게 하기 위해 
            //판단제어구조가 필요하다
            //<--판단제어구조는 셰이더에 적합치 않으므로 step함수를 사용하겠다 
            float tOutlineResult= step(_OutlineWidth, tDotResult);            //tDotResult>=_OutlineWidth ? 1 : 0

            tResult.rgb = s.Albedo * tOutlineResult;    //tOutlineResult이 1이면 그냥 색상, 0이면 검정색상
            tResult.a = s.Alpha;
