/*
    ...
    Ramp Texture기법 
        <-- '텍스처(이미지) 기반 라이팅 기법' 이다.

    Diffuse Warp 기법

        <-- 밸브 사가 발표한 팀포트리스2의 그래픽 톤앤매너를 만들어내기 위한 방법 들 중 하나
        <-- '텍스처(이미지) 기반 라이팅 기법' 이다.
...
Shader "Ryu/shDiffuseWarp_4"
{
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
            ...
            fixed4 c = tex2D(_MainTex, IN.uv_MainTex);//fixed4(1,1,1,1);//<--흰색으로 결정. 왜냐하면 그래야 조명색상을 정확히 관찰가능

            //노멀맵텍스처에서 텍셀을 가져와, 접선공간으로 끄집어 내기(법선벡터로 바꾸는) 위한 것이 필요하다.
            //UnpackNormal
        ...
        float4 LightingDiffuseWarp(SurfaceOutput s, float3 lightDir, float3 viewDir ,float atten )
        {
            ...
            //ramp texture의 uv중에서 v를 임의의 기준값으로 판단하여 림라이트를 이용한 외곽선 구간을 만들었다.
